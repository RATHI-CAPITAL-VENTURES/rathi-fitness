...
| Things for today only | Today, under the header — **Locker**, **Parking**, a **Note**, or **Other** with your own heading. Filled chips read as themselves ("Locker 214"); tomorrow they are outlines again, with nothing to clear. Past workouts keep what you noted. `gym today` prints them, so RIA can tell you your locker. No chip for the combination, deliberately — see `docs/SNAPSHOT.md`. |
| Look back at past sessions | Swipe left on Today. Read-only summaries, newest first — see below. |
| Set / cooldown counter per exercise | The set screen, pushed from a Today row. At its foot: the last three times you did this exercise, and **its trend** — the same chart the Trends tab draws, moving as you log. Weight for a lift, least help on an assisted machine, best set of reps for a bodyweight lift, miles (else minutes) for cardio — which has a trend here and nowhere else, since the Trends tab's picker is lifts only. |
...
| Something else, today only | **Hold an exercise on Today → Do something else today.** The treadmills are taken, so you ride the bike: the row becomes the bike "for Treadmill", riding it ticks the slot off, and tomorrow the plan is the treadmill again with nothing to undo. Sets, reps, rest and minutes carry over; weight, miles and grade do not — those were facts about the other machine. Sets you had already done in the slot still count: two on the bench and two on the dumbbells is four of four. Swap a lift for cardio (or the reverse) and it opens like a new slot, on your defaults. The picker puts what you usually do instead first, then anything that does the same job, and reaches into the catalogue for machines you have never logged. |
| Time in the gym | First log to last — on Today while you train, on every past workout, and as a lifetime total on Trends. A workout that **opens** with cardio counts the bout's own length, because a treadmill is logged when you step off, not on. `gym sessions` prints it per workout and all told. |
...
| Bars | Exercise editor → Bar. 45 standard, 35 women's, **25 short**, 15 technique, or none — and "A different bar" to type the weight of a hex, EZ-curl or Smith bar, none of which weighs the same in two gyms. Plate math and the steppers follow whichever is set. |
...
| Glasses | **Meta Ray-Ban Display.** Settings → Glasses. Open the app at the gym, put the phone away, and the lens runs the workout: **today's exercises as a list** (thumb-swipe to move, pinch to open — what you are part-way through is already lit), a card with what to lift and where the seat goes, then **Log set** and the rest counting down, ember cooling to teal, then what is next. **−1 rep** for the set you fell short on; **Taken** offers what could stand in, the same ranking the phone's picker uses. Open an exercise on the phone and the lens mirrors that instead. Lifts only from the lens — a treadmill's numbers come off its console and the lens cannot type — so a machine's card sends you to the phone. It takes the lens only around a workout, never while you are just wearing your glasses. Taking them off drops the connection and putting them back restores it in seconds; the phone keeps the workout either way. Needs Developer Mode in the Meta AI app, and is off until you switch it on — while it is off the app never calls Meta's code. See `docs/DECISIONS.md`, 2026-09-21. |
